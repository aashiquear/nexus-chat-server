...
function ResponseTimer({ startedAt, durationMs }) {
  ...
  return (
    <div
      className={`response-timer ${isDone ? "is-final" : "is-live"}`}
...
function ThinkingBlock({ text, complete }) {
  ...
  return (
    <div
      className={`thinking-block ${complete ? "thinking-complete" : "thinking-active"}`}
    >
      <button
...
function getFileTypeMeta(filename, contentType) {
  ...
  if (isPdf) {
    ...
    className = "file-icon-sheet";
  } else if (isCode || isJson) {
    icon = <FileCodeIcon size={14} />;
    className = "file-icon-code";
  } else if (isMd || isHtml || isText) {
    icon = <FileTextIcon size={14} />;
    className = "file-icon-text";
  }

  const previewable =
    isPdf || isImg || isHtml || isMd || isJson || isText || isCode;
...
function CodeBlock({ code, lang, onOpenCanvas }) {
  ...
  const isKnownLang = !!(
    lang && LANG_EXT_MAP[(lang || "").toLowerCase().trim()]
  );
  ...
  return (
    <div className="code-block-wrapper">
      ...
      <pre
        className={`code-block-pre ${isLarge && !expanded ? "truncated" : ""}`}
      >
...
function formatInline(text) {
  return text
    ...
    .replace(
      /!\[([^\]]*)\]\(([^)]+)\)/g,
      '<img src="$2" alt="$1" class="md-inline-img" />',
    )
  ...
  return parts.map((part, i) => {
    ...
    while (idx < textLines.length) {
        ...
        elements.push(
          <blockquote
            ...
            dangerouslySetInnerHTML={{
              __html: quoteLines.map(formatInline).join("<br/>"),
            }}
          />,
        );
        continue;
      ...
      if (
        ...
        const allBullets =
          splitLines.length > 1 && splitLines.every((l) => BULLET_RE.test(l));
        if (allBullets) {
          elements.push(
        ...
        const allOrdered =
          splitLines.length > 1 && splitLines.every((l) => ORDERED_RE.test(l));
        if (allOrdered) {
          elements.push(
...
function buildCanvasDataForResult(toolName, parsedResult) {
  ...
  if (parsedResult.figure_json) {
    ...
    return {
      figureJson: fig,
      title:
        parsedResult.title || fig?.layout?.title?.text || "Interactive Plot",
    };
  }
  if (parsedResult.plot_image) {
    return {
      image: parsedResult.plot_image,
      title: parsedResult.title || "Generated Plot",
    };
  }
  ...
  if (toolName === "code_executor" && parsedResult.output != null) {
    return {
      title: "Code Output",
      contentType: "terminal",
      ...
      {isOpen && (
        <div className="tool-dropdown-body">
          ...
          {toolResult && (
            <div className="tool-dropdown-section">
              <div
                className="tool-dropdown-section-label"
                style={{ color: hasError ? "var(--error)" : "var(--success)" }}
              >
      ...
      <div className="plot-result-header">
        ...
        <div className="plot-result-actions">
          <button
            ...
            onClick={() =>
              onOpenCanvas && onOpenCanvas({ image: imageName, title })
            }
      ...
      <div className="plot-result-header">
        ...
        <div className="plot-result-actions">
          <button
            className="plot-result-action"
            onClick={() =>
              onOpenCanvas && onOpenCanvas({ figureJson: figData, title })
            }
            title="Preview in canvas"
          >
            <EyeIcon size={12} />
...
// Terminal output card: compact inline preview of code execution
// results with preview / download / copy actions, mirroring the
// plot-result cards.
function TerminalOutputCard({ output, title, onOpenCanvas }) {
  const [copied, setCopied] = useState(false);
  ...
  return (
    <div className="plot-result-card">
      ...
      <div className="terminal-container" style={{ maxHeight: "200px" }}>
      ...
      <div className="plot-result-footer">
        <span className="plot-result-meta">{chunks.length} lines</span>
      </div>
    </div>
  );
}

// Sandbox session card: shows an interactive session is ready with a
// button to reopen it in the canvas terminal.
function SandboxSessionCard({ sessionId, onOpenCanvas }) {
  return (
    <div className="plot-result-card">
      <div className="plot-result-header">
        <TerminalIcon size={14} />
      ...
      >
        Interactive sandbox session ready.
      </div>
      <div className="plot-result-footer">
        <span className="plot-result-meta">Session {sessionId}</span>
      </div>
    </div>
  );
}
...
function DownloadableFileCard({ file, onOpenCanvas }) {
  ...
  const handleCopy = useCallback(async () => {
    try {
      if (isImg && previewHref) {
        const blob = await fetch(previewHref).then((r) => r.blob());
        if (navigator.clipboard && window.ClipboardItem) {
          await navigator.clipboard.write([
            new ClipboardItem({ [blob.type]: blob }),
          ]);
  ...
  }, [isImg, previewHref, downloadHref]);
...
export default function ChatMessage({ message, onOpenCanvas }) {
  ...
  // Find any plot, SVG, downloadable, sandbox, or code results for inline display
  const specialResults = toolResults.filter((tr) => {
    try {
      const p = JSON.parse(tr.result);
      return (
        (p && p.svg && p.svg.includes("<svg")) ||
        (p && p.plot_image) ||
        (p && p.figure_json) ||
        (p && p.downloadable) ||
        (p && p.interactive_session) ||
        (p && p.output != null)
      );
    } catch (_e) {
      return false;
        ...
        {specialResults.map((tr, i) => {
          ...
          if (parsed.figure_json) {
            ...
            if (typeof parsed.figure_json === "string") {
              ...
              typeof parsed.figure_json === "object" &&
              parsed.figure_json !== null
            ) {
            ...
            if (!figData || typeof figData !== "object") {
              return (
                <div
                  key={`plotly-${i}`}
                  className="plot-result-card plotly-result-card"
                >
                  <div className="plot-result-header">
                    <BarChart3 size={14} />
            ...
            const plotTitle =
              parsed.title || figData.layout?.title?.text || "Interactive Plot";
            return (
              <PlotlyChartCard
                key={`plotly-${i}`}
                figData={figData}
          ...
          if (parsed.output != null && tr.name === "code_executor") {
            return (
              <TerminalOutputCard
                key={`code-${i}`}
          ...
          if (parsed.interactive_session) {
            return (
              <SandboxSessionCard
                key={`sandbox-${i}`}
                sessionId={parsed.interactive_session}
                onOpenCanvas={onOpenCanvas}
              />
            );
          }
...
export function TypingIndicator() {
  return (
    <div className="message">
      ...
      <div
        className="typing-indicator typing-indicator-connected"
